...
# класс питчера
class Pitcher(Player, Calculate, Printable):
    ...
    def to_string(self):
        return f"Питчер: {self._name}, команда: {self._team}, avg skills: {self.calculate()}"
...
# класс зашиты
class Defensive(Player, Calculate, Printable):
    ...
    def to_string(self):
        return f"{self._position}: {self._name}, команда: {self._team}, sum stats: {self.calculate()}"
