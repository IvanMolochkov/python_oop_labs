...
from src.lab01.validate import (validate_name, validate_age, validate_team, validate_batting_average, validate_home_runs, validate_position, validate_status)
...
class Player(ABC):
    ...
    def __str__(self):
        return (
            f"name: {self._name}, age: {self._age}, team: {self._team}, position: {self._position}, BA: {self._batting_avg:.3f}, HR: {self._home_runs}, is playing: {self._is_playing}"
        )
 
    def __repr__(self):
        return (
            f"Player(name={self._name!r}, age={self._age}, team={self._team}, batting_avg={self._batting_avg:.3f}, home_runs={self._home_runs}, position={self._position!r}, is playing={self._is_playing!r})"
        )
